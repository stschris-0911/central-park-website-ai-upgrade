...
model = YOLO("crosswalk.pt").to(device)
...
# ========== 类别和颜色 ==========
# ✅ 新模型：5 类，crosswalk 新增在索引 0
classNames = ["crosswalk", "curb_down", "curb_up", "road", "sidewalk"]

class_colors = {
    0: (255, 255, 255),  # crosswalk → 白色（斑马线本身就是白色）
    1: (0, 0, 255),      # curb_down → 红色
    2: (0, 255, 255),    # curb_up   → 黄色
    3: (0, 255, 0),      # road      → 绿色
    4: (255, 0, 255)     # sidewalk  → 品红
}
...
# ========== ✅ 新增：CrosswalkCenteringAnalyzer 类 ==========
class CrosswalkCenteringAnalyzer:
    """
    过马路居中分析：根据 crosswalk mask 计算用户相对斑马线中心的偏移，
    给出 center / slight_left / slight_right / turn_left / turn_right 引导。

    关键设计：
    - 只分析画面下半部分（用户近处），减少远处 mask 的影响
    - 每行取左右边界的中点，而非整体质心 → 抗形状不规则
    - 距离加权（底部权重更大）→ 近处行为主导
    """

    # 档位阈值（单位：offset_ratio，范围 [-1, 1]）
    CENTER_THRESHOLD = 0.10        # < 0.10 视为居中
    SLIGHT_THRESHOLD = 0.25        # 0.10 ~ 0.25 为轻微偏离，> 0.25 为严重偏离

    # 激活阈值
    MIN_AREA_RATIO = 0.03          # crosswalk 占画面至少 3%
    MIN_VALID_ROWS = 3             # 下半部至少 3 行有有效 crosswalk

    # 采样参数
    NUM_SCAN_LINES = 8             # 下半部取 8 条扫描线
    ANALYSIS_Y_START_RATIO = 0.50  # 从画面 50% 高度开始分析
    ...
    def __init__(self, frame_width, frame_height):
        ...
        logger.info(f"CrosswalkCenteringAnalyzer initialized:")
        logger.info(f"  User center x: {self.user_center_x}")
        logger.info(f"  Analysis Y: [{self.analysis_y_start}:{self.analysis_y_end}]")
        logger.info(f"  Scan lines: {self.NUM_SCAN_LINES}")

    def analyze(self, crosswalk_mask, crosswalk_area_ratio):
        """
        分析斑马线，返回居中引导信息。

        Args:
            crosswalk_mask: crosswalk 的 mask（任意大小，会 resize）
            crosswalk_area_ratio: crosswalk 面积占比（已在主循环计算）

        Returns:
            dict with keys:
              - activated: bool，是否激活 crosswalk 模式
              - offset_ratio: float in [-1, 1]，中心偏移比例
              - direction: 'center' | 'left' | 'right' | None
              - intensity: 'none' | 'weak' | 'strong'
              - crosswalk_center_x: 综合中心 x（像素）
              - row_centers: list of (y, center_x, width)，每行的中心点
              - valid_row_count: 有效行数量
        """
        result = {
            'activated': False,
            'offset_ratio': 0.0,
        ...
        # ---- 逐行扫描，计算每行的斑马线中心 ----
        row_centers = []  # [(y, center_x, width), ...]
        weights = []

        for y in self.scan_ys:
            row = mask_binary[y, :]
            xs = np.where(row > 0)[0]
            if len(xs) < 5:  # 太少像素视为无效行
                continue
            ...
            # 距离权重：y 越大（越靠近底部/用户）权重越大
            # 归一化 y 到 [0, 1]：y_start=0, y_end=1
            y_norm = (y - self.analysis_y_start) / max(1, self.analysis_y_end - self.analysis_y_start - 1)
            weight = y_norm + 0.5  # 最上面权重 0.5，最下面权重 1.5

            row_centers.append((int(y), center_x, width))
            weights.append(weight)

        result['row_centers'] = row_centers
        result['valid_row_count'] = len(row_centers)
        ...
        # ---- 加权平均计算综合中心 ----
        centers_arr = np.array([c[1] for c in row_centers])
        weights_arr = np.array(weights)
        crosswalk_center_x = float(np.sum(centers_arr * weights_arr) / np.sum(weights_arr))

        # ---- 计算偏离 ----
        offset = crosswalk_center_x - self.user_center_x
        offset_ratio = offset / self.half_width
        # 夹到 [-1, 1]
        ...
        if abs_ratio < self.CENTER_THRESHOLD:
            direction = 'center'
            intensity = 'weak'  # 居中时可静默或轻振（主循环再决定是否发语音）
    ...
    def get_haptic_signal(self, offset_ratio, intensity, direction):
        """
        映射到 5 列 haptic belt。
        Returns: (col_idx, top_intensity, bottom_intensity) 或 None
        """
        if direction is None or intensity == 'none':
            return None
        ...
        # 偏左 → 振动左列（col 0 / 1），提示用户向左移动
        # 偏右 → 振动右列（col 3 / 4），提示用户向右移动
        if direction == 'left':
            col = 0 if intensity == 'strong' else 1
        else:  # right
            col = 4 if intensity == 'strong' else 3
    ...
    def visualize(self, frame, analysis_result):
        """在画面上可视化斑马线中心、用户位置、偏移箭头。"""
        if not analysis_result['activated']:
            return

        row_centers = analysis_result['row_centers']
        crosswalk_center_x = analysis_result['crosswalk_center_x']
        offset_ratio = analysis_result['offset_ratio']
        direction = analysis_result['direction']
        intensity = analysis_result['intensity']

        # 1) 每行中心点连线（黄色折线）
        if len(row_centers) >= 2:
            pts = np.array([(int(cx), int(y)) for (y, cx, _) in row_centers], dtype=np.int32)
            for i in range(1, len(pts)):
                cv2.line(frame, tuple(pts[i - 1]), tuple(pts[i]), (0, 255, 255), 2)
            # 画出每行的点
            for (y, cx, w) in row_centers:
                cv2.circle(frame, (int(cx), int(y)), 3, (0, 255, 255), -1)

        # 2) 综合中心（青色竖线）
        cc_x = int(crosswalk_center_x)
        cv2.line(frame, (cc_x, self.analysis_y_start),
                 (cc_x, self.analysis_y_end - 1), (255, 255, 0), 2)

        # 3) 用户中心（绿色竖线）
        uc_x = int(self.user_center_x)
        cv2.line(frame, (uc_x, self.analysis_y_start),
                 (uc_x, self.analysis_y_end - 1), (0, 255, 0), 2)

        # 4) 偏移箭头（从用户中心指向斑马线中心，在画面中下部）
        arrow_y = int(self.frame_height * 0.85)
        arrow_color = (0, 0, 255) if intensity == 'strong' else (0, 200, 255)
        if abs(cc_x - uc_x) > 3:  # 避免重叠
            cv2.arrowedLine(frame, (uc_x, arrow_y), (cc_x, arrow_y),
        ...
        if direction == 'center':
            ...
            arrow_char = "->" if direction == 'right' else "<-"
            strength_str = "TURN" if intensity == 'strong' else "slight"
            label = f"CW: offset={offset_ratio:+.2f} {arrow_char} {strength_str} {direction}"
            label_color = (0, 0, 255) if intensity == 'strong' else (0, 200, 255)

        cv2.putText(frame, label, (10, 85),
...
class CurbGuidanceTracker:
    def __init__(self, stability_frames=3, cooldown_seconds=2, angle_tolerance=15):  # ✅ 改为3帧
        ...
        # ✅ ========== Crosswalk centering 追踪 ==========
        # 方向历史（'center' / 'left' / 'right' / None）
        self.crosswalk_direction_history = deque(maxlen=stability_frames)
        # 偏移比例历史（用于平滑）
        self.crosswalk_offset_history = deque(maxlen=stability_frames)
        # 强度历史
        self.crosswalk_intensity_history = deque(maxlen=stability_frames)
        # 是否激活（用于优先级判断）
        self.crosswalk_activated_history = deque(maxlen=stability_frames)
        self.crosswalk_active_stable = False  # 连续激活是否稳定
        self.last_crosswalk_guidance_time = 0
        self.crosswalk_guidance_cooldown = 2.0  # ✅ 比 open path 短，过马路需要及时反馈
    ...
    # ========== ✅ Crosswalk centering 相关方法 ==========
    def update_crosswalk(self, analysis_result):
        """用每一帧的 crosswalk 分析结果更新历史。"""
        activated = analysis_result['activated']
        self.crosswalk_activated_history.append(activated)

        if activated:
            self.crosswalk_direction_history.append(analysis_result['direction'])
            self.crosswalk_offset_history.append(analysis_result['offset_ratio'])
            self.crosswalk_intensity_history.append(analysis_result['intensity'])
        else:
            self.crosswalk_direction_history.append(None)
            self.crosswalk_offset_history.append(0.0)
            self.crosswalk_intensity_history.append('none')

        # 稳定激活：最近 3 帧中至少 2 帧激活
        active_count = sum(1 for a in self.crosswalk_activated_history if a)
        self.crosswalk_active_stable = (
            len(self.crosswalk_activated_history) == self.stability_frames and
            active_count >= max(2, int(self.stability_frames * 0.6))
        )

    def is_crosswalk_active_stable(self):
        """判断 crosswalk 模式是否稳定激活（用于优先级判断）。"""
        return self.crosswalk_active_stable

    def should_guide_crosswalk(self, current_time):
        """
        判断是否应该发出 crosswalk 居中引导。
        Returns: (should_guide: bool, stable_direction: str, stable_intensity: str, smoothed_offset: float)
        """
        time_since_last = current_time - self.last_crosswalk_guidance_time
        if time_since_last < self.crosswalk_guidance_cooldown:
            return False, None, 'none', 0.0

        if not self.crosswalk_active_stable:
            return False, None, 'none', 0.0

        # 过滤掉 None
        valid_dirs = [d for d in self.crosswalk_direction_history if d is not None]
        valid_offsets = [o for o in self.crosswalk_offset_history if o != 0.0 or len(valid_dirs) > 0]
        valid_intensities = [i for i in self.crosswalk_intensity_history if i != 'none']

        if len(valid_dirs) < max(2, int(self.stability_frames * 0.6)):
            return False, None, 'none', 0.0

        # 方向：众数
        from collections import Counter
        direction_counts = Counter(valid_dirs)
        stable_direction, direction_votes = direction_counts.most_common(1)[0]
        # 至少过半票才算稳定
        if direction_votes < max(2, int(len(valid_dirs) * 0.5)):
            return False, None, 'none', 0.0

        # offset：中位数（抗抖动）
        smoothed_offset = float(np.median(valid_offsets)) if valid_offsets else 0.0

        # 强度：众数（strong 比 weak 优先，遇到平手取 strong）
        if 'strong' in valid_intensities:
            strong_cnt = valid_intensities.count('strong')
            if strong_cnt >= len(valid_intensities) * 0.5:
                stable_intensity = 'strong'
            else:
                stable_intensity = 'weak'
        else:
            stable_intensity = 'weak'

        return True, stable_direction, stable_intensity, smoothed_offset

    def mark_crosswalk_guidance_given(self, current_time):
        self.last_crosswalk_guidance_time = current_time
...
# ✅ 初始化 CrosswalkCenteringAnalyzer
crosswalk_analyzer = CrosswalkCenteringAnalyzer(frame_width, frame_height)
...
# ========== 帧计数器：统计每个class出现的帧数 ==========
# ✅ 新模型 5 类：0: crosswalk, 1: curb_down, 2: curb_up, 3: road, 4: sidewalk
class_frame_counts = {0: 0, 1: 0, 2: 0, 3: 0, 4: 0}
...
try:
    while not exit_flag.is_set():
        ...
        # ✅ Crosswalk 分析变量
        crosswalk_mask = None
        crosswalk_analysis = None  # CrosswalkCenteringAnalyzer.analyze 返回的 dict
        ...
        with profile_code_block("04_YOLO结果处理"):
            for result in results:
                if result.masks is not None:
                    ...
                    # ✅ 新索引：crosswalk=0, road=3, sidewalk=4
                    # 计算面积并获取 sidewalk / crosswalk mask
                    for i, cls in enumerate(result_classes):
                        if result_confidences[i] >= confidence_threshold:
                            ...
                            if cls == 0:  # ✅ crosswalk
                                if VIDEO_TEST_MODE:
                                    crosswalk_area += np.sum(mask_resized > 0)
                                    if crosswalk_mask is None:
                                        crosswalk_mask = mask_resized.copy()
                                    else:
                                        crosswalk_mask = np.maximum(crosswalk_mask, mask_resized)
                                else:
                                    crosswalk_area += np.sum(mask)
                                    if crosswalk_mask is None:
                                        crosswalk_mask = mask.copy()
                                    else:
        ...
        # ===== ✅ 5a. Crosswalk Centering 分析（优先级高于 Open Path）=====
        with profile_code_block("05a_crosswalk_analysis"):
            # 每帧都跑分析（用于更新 history）；即使未激活也要调用 update 维持 deque
            crosswalk_analysis = crosswalk_analyzer.analyze(
                crosswalk_mask, crosswalk_area
            )
            # 更新追踪器
            curb_tracker.update_crosswalk(crosswalk_analysis)

            # 可视化（激活时才画）
            if crosswalk_analysis['activated']:
                crosswalk_analyzer.visualize(dst, crosswalk_analysis)

                if frame_count % 30 == 0:
                    logger.info(f"🚶 Crosswalk Analysis:")
                    logger.info(f"   Area: {crosswalk_area:.2%}, Valid rows: {crosswalk_analysis['valid_row_count']}")
                    logger.info(f"   Offset ratio: {crosswalk_analysis['offset_ratio']:+.3f}")
                    logger.info(f"   Direction: {crosswalk_analysis['direction']}, Intensity: {crosswalk_analysis['intensity']}")
        ...
        with profile_code_block("05_open_path_analysis"):
            ...
            # ✅ 优先级控制：只有 crosswalk 模式**未稳定激活**时才运行 Open Path
            crosswalk_mode_active = curb_tracker.is_crosswalk_active_stable()

            if not crosswalk_mode_active and sidewalk_mask is not None and sidewalk_area > 0.05:
                # ✅ 接收所有返回值
        ...
        with profile_code_block("12_TTS_open_path"):
            ...
            # ✅ 优先级：curb 在扇形区域内 **或** crosswalk 稳定激活 时，都不给 open path 引导
            crosswalk_mode_active = curb_tracker.is_crosswalk_active_stable()
            if not (curb_down_in_fan or curb_up_in_fan) and not crosswalk_mode_active:
        ...
        # ===== ✅ 13b. Crosswalk Centering 语音 + 触觉引导 =====
        with profile_code_block("12b_TTS_crosswalk"):
            # 优先级：curb 在扇形区域内时，不给 crosswalk 引导（curb warning 最高优先）
            if crosswalk_mode_active and not (curb_down_in_fan or curb_up_in_fan):
                should_guide_cw, stable_cw_dir, stable_cw_int, smoothed_offset = \
                    curb_tracker.should_guide_crosswalk(current_time)

                # --- 触觉反馈（每帧，不等冷却，需要实时）---
                if HAPTIC_ENABLED:
                    # 直接用当前帧的分析结果驱动 haptic（更实时）
                    if crosswalk_analysis and crosswalk_analysis['activated']:
                        cw_haptic = crosswalk_analyzer.get_haptic_signal(
                            crosswalk_analysis['offset_ratio'],
                            crosswalk_analysis['intensity'],
                            crosswalk_analysis['direction']
                        )
                        if cw_haptic is not None:
                            col_idx, top_int, bot_int = cw_haptic
                            haptic_controller.set_column_vibration(col_idx, top_int, bot_int)
                            if frame_count % 30 == 0:
                                logger.info(f"🎮 CW Haptic: col={col_idx}, top={top_int}, bot={bot_int}")
                        else:
                            haptic_controller.clear_all()

                # --- TTS 语音（受冷却限制）---
                if should_guide_cw and stable_cw_dir is not None:
                    cw_text = None
                    if stable_cw_dir == 'center':
                        cw_text = "go_straight"
                    elif stable_cw_dir == 'left':
                        cw_text = "turn_left" if stable_cw_int == 'strong' else "slight_left"
                    elif stable_cw_dir == 'right':
                        cw_text = "turn_right" if stable_cw_int == 'strong' else "slight_right"

                    if cw_text:
                        mimic3_tts_thread.enqueue_phrase(cw_text, priority=False)
                        curb_tracker.mark_crosswalk_guidance_given(current_time)
                        logger.info(f"🚶 Crosswalk TTS: {cw_text} "
                                    f"(dir={stable_cw_dir}, int={stable_cw_int}, offset={smoothed_offset:+.2f})")

            elif crosswalk_mode_active and (curb_down_in_fan or curb_up_in_fan):
                # curb warning 优先，清掉 haptic（让 curb beep/warning 接管）
                if frame_count % 60 == 0:
                    logger.info("🚫 Crosswalk guidance suppressed by curb warning")
        ...
        with profile_code_block("13_可视化"):
            ...
            # ✅ Crosswalk 模式状态
            cw_mode = "CW:ON" if curb_tracker.is_crosswalk_active_stable() else "CW:--"
            
            status_text = f"FPS:{fps} | {mode_text} | Beep:{beep_status} | Path:{open_path_status} | {sidewalk_status} | {cw_mode}"
            cv2.putText(dst, status_text, (10, frame_height - 20), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
            
            # ✅ 显示 road / sidewalk / crosswalk 面积
            traversable_text = f"Road:{road_area:.1%} | SW:{sidewalk_area:.1%} | CW:{crosswalk_area:.1%}"
            cv2.putText(dst, traversable_text, (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            
            # ========== 右上角：帧统计（5 类）==========
            # ✅ 索引：0=crosswalk, 1=curb_down, 2=curb_up, 3=road, 4=sidewalk
            stats_lines = [
                f"Total:{frame_count + 1}",
                f"crosswlk:{class_frame_counts[0]}",
                f"curb_dn: {class_frame_counts[1]}",
                f"curb_up: {class_frame_counts[2]}",
                f"road:    {class_frame_counts[3]}",
                f"sidwlk:  {class_frame_counts[4]}",
            ]
