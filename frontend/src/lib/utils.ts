...
export function cleanCoordinateLabel(label?: string | null): string {
  const value = String(label ?? "").trim();
  if (!value) return "";

  const coordinatePair = "-?\\d{1,3}(?:\\.\\d+)?\\s*,\\s*-?\\d{1,3}(?:\\.\\d+)?";
  const coordinateSuffix = new RegExp(`\\s*\\(${coordinatePair}\\)\\s*$`);
  const coordinateOnly = new RegExp(`^\\(?\\s*${coordinatePair}\\s*\\)?$`);

  if (coordinateOnly.test(value)) {
    return "Selected point";
  }

  return value.replace(coordinateSuffix, "").trim();
}

export function endpointLabel(endpoint?: RouteEndpointInfo | null): string {
  return cleanCoordinateLabel(endpoint?.label);
}
