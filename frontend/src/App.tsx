...
function metersBetweenLatLon(a: [number, number], b: [number, number]) {
  const r = 6371000;
...
function angleAtLatLon(a: [number, number], b: [number, number], c: [number, number]) {
  const v1x = b[1] - a[1];
  const v1y = b[0] - a[0];
  const v2x = c[1] - b[1];
  const v2y = c[0] - b[0];
...
function addRouteReminderNodes(
  originalNodes: RoutePathNode[],
  routeCoordsLatLon: [number, number][]
): RoutePathNode[] {
  if (!routeCoordsLatLon || routeCoordsLatLon.length < 3) return originalNodes;

  const existingLatLon = originalNodes
    .filter((node) => node.point && node.point.length >= 2)
    .map((node) => [Number(node.point![1]), Number(node.point![0])] as [number, number]);

  const reminders: RoutePathNode[] = [];
  const reminderLatLon: [number, number][] = [];

  function nearExistingNode(pt: [number, number]) {
    return existingLatLon.some((p) => metersBetweenLatLon(pt, p) < 35);
  }

  function nearExistingReminder(pt: [number, number]) {
    return reminderLatLon.some((p) => metersBetweenLatLon(pt, p) < 85);
  }

  function addReminder(pt: [number, number], kind: "long" | "turn") {
    if (nearExistingNode(pt)) return;
    if (nearExistingReminder(pt)) return;

    const index = reminders.length + 1;

    reminders.push({
      node_id: `route_waypoint:${index}`,
      label: kind === "turn" ? `Turn reminder ${index}` : `Long segment reminder ${index}`,
      point: [pt[1], pt[0]],
      category: "route_waypoint",
      code: null,
      description:
        kind === "turn"
          ? "Temporary reminder added because this section has a noticeable turn."
          : "Temporary reminder added because this section continues for a relatively long distance."
    });

    reminderLatLon.push(pt);
  }

  let runningDistance = 0;

  for (let i = 1; i < routeCoordsLatLon.length - 1; i++) {
    const prev = routeCoordsLatLon[i - 1];
    const curr = routeCoordsLatLon[i];
    const next = routeCoordsLatLon[i + 1];

    runningDistance += metersBetweenLatLon(prev, curr);

    const angle = angleAtLatLon(prev, curr, next);

    if (angle >= 30) {
      addReminder(curr, "turn");
      runningDistance = 0;
      continue;
    }

    if (runningDistance >= 100) {
      addReminder(curr, "long");
      runningDistance = 0;
    }
  }

  return [...originalNodes, ...reminders];
}
...
export default function App() {
  ...
  function applyRoute(route: RouteResponse) {
    const coords = cleanRouteCoords(route.route_geojson.geometry.coordinates.map(([lon, lat]) => [lat, lon] as [number, number]));
    ...
    setPathNodes(addRouteReminderNodes(route.path_nodes ?? [], coords));
